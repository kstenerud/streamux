...
A minimalist, multiplexing, bi-directional messaging and streaming protocol.
...
If a peer detects an error in the initial message, it must close the connection, or discard all messages if the connection cannot be closed.
...
The reply bit is used to reply to a message sent by a peer. When set, the ID field refers to the ID of a message sent by the peer.
...
Sent in response to a cancel request. All queued replies to that ID are removed, and the operation is canceled. Once the cancel is completed, send Cancel Ack.

#### Keep Alive

Each side keeps track of how long it's been idle (no messages in the send queue). After a threshold idle time, send a keep alive. Use next free ID.
...
Messages are sent in chunks. A multi-chunk message has its termination bit cleared for all but the last chunk. Single chunk messages always have the termination bit set.

The message ID range is scoped to the sender. If both sides of the conversation send a message with the same ID, they are considered different messages, and don't interfere with each other. A peer replies to a message by using the same message ID and setting the reply bit.
...
As this is a multiplexed protocol, responses may be sent in a different order than the requests were received.


### Multiplexing

Longer messages do not have to be sent all at once. Message chunks are assumed to be sent in-order, and complete once the termination bit is set. This means that you can intersperse chunks from different messages:
...
A message must receive a response within a reasonable time. Each peer can tell the other what their timeout expectations are in their initial message.
...
If a peer has not received any messages within the timeout period, it may assume that the sender is lagged. Timeouts should be adjusted to compensate for lag.
...
In the error case, the peer may elect to send an error message or end the connection, depending on your use case.
