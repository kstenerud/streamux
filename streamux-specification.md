...
The [ID cap field](#_id_cap) determines the maximum allowed value of this field for the current session.
...
* When a peer proposes a negotiation mode other than `passive`, its own [allowed negotiation modes](#_allowed_modes) list is ignored.
...
This is used to negotiate the [ID cap](#_id_cap) and [length cap](#_length_cap).
...
This is used to negotiate [fixed length](#_fixed_length) and [padding](#_padding).
